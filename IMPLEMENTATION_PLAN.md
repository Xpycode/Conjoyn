...
| 6.5 | ⏳ **Footage-gated** Variant-guard + edge cases | — | `_W/_Z/_T` never merged; missing-middle splits; mixed-codec refused; trailing tiny segment included; VFR fallback. **Blocked: needs a multi-lens drone** — Mini 4 Pro is single-camera (all clips `_D`), structurally can't exercise this. **Acquire to close:** (a) Mavic 3 Pro wide+tele recorded in one session → variant no-merge; (b) a mixed-codec pair (H.264 + HEVC) → refusal; (c) a split recording with a deliberately-deleted middle segment → gap handling | matrix of fixtures passes |
