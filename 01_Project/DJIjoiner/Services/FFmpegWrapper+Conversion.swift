...
extension FFmpegWrapper {
    ...
    /// Output-side metadata stamped onto the joined file during the join. The grouping/metadata
    /// stages (tasks 2.4 / 2.8) supply these from segment 1; both are optional so the builder
    /// stays usable before those land.
    ...
    /// ```
    /// -f concat -safe 0 -i <list> -map 0:v -map 0:a? -map -0:d -c copy \
    ///   -fflags +genpts -movflags +faststart [-metadata creation_time=…] [-timecode …] -y <out>
    /// ```
    /// - `-map 0:v -map 0:a?` keeps video and (optional) audio; `-map -0:d` drops in-container
    ///   data streams (DJI telemetry data tracks) that would otherwise break `-c copy`.
