...
A split recording is two or more files that came from one continuous press of the record button. DJI cameras split at around 4 GB to stay within the FAT32 filesystem limit. GoPro writes far larger pieces — it calls them chapters — and the size it splits at shifts with the frame rate you shot at. Either way the pieces are one take.
...
A single recording is a lone clip that never reached its camera's split size. No joining is necessary, but you can still queue a single to apply the recording-date fix, stamp a timecode track, and carry over its `.SRT` sidecar.
...
Clips recorded at a high frame rate (e.g. 100 fps) are stored in a 25 fps container — so the file plays back at 4× the real recording time. Conjoyn detects this from the `.SRT` telemetry and shows a **slow-mo** badge. Because the detection reads the sidecar, it applies to DJI footage only. The timecode is stamped at the real recording start and advances at the file's playback rate (25 fps), which is what NLEs expect.
